    ...
    always @(*) begin
        ...
        if (rst == `RstEnable) begin
            ...
            case (opcode)
                `INST_TYPE_I: begin     //I类指令
                    case(funct3)
                        ...
                        `EXE_SLTI, `EXE_SLTIU: begin
                            ...
                            imm         <= {{20{inst_i[31]}}, inst_i[31:20]};  //符号扩展立即数
                            wd_o        <= rd;              //写回的寄存器地址
                        end
                        default: begin
                ...
                `EXE_LUI: begin
                    wreg_o      <= `WriteEnable;    //写回使能
                    reg1_read_o <= 1'b0;            //不需要读寄存器
                    reg2_read_o <= 1'b0;            //不需要读寄存器
                    instvalid   <= `InstValid;      //指令有效
                    imm         <= {inst_i[31:12], 12'b0}; //立即数左移12位
                    wd_o        <= rd;              //写回寄存器地址
                end
                // ===== AUIPC 指令 =====
                `EXE_AUIPC: begin
                ...
                `INST_TYPE_SYS: begin
                    wreg_o      <= `WriteDisable;   //简化实现：暂时不写回
                    reg1_read_o <= 1'b0;
                    reg2_read_o <= 1'b0;
                    instvalid   <= `InstValid;      //指令有效，简化处理
                end
            endcase
        end     //if
    end     //always
